...
import { currentPayPeriod, fourteenDayPeriodStarting } from "@/lib/time-card/payroll";
...
export async function GET(request: Request) {
  ...
  let period;
  try { period = raw ? fourteenDayPeriodStarting(raw) : currentPayPeriod(); } catch { period = currentPayPeriod(); }
