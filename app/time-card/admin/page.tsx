...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ period?: string; employee?: string; history?: string; error?: string; success?: string }> }) {
  ...
  return <main className="time-card-shell time-card-admin">
    <header className="time-card-topbar"><div><p className="time-card-eyebrow">Relief Plus</p><strong>Payroll Administration</strong></div><div className="time-card-utilities"><Link className="time-card-return-link" href="/">← Back to Relief Plus</Link><form action={logoutAction}><button className="time-card-text-button">Sign out</button></form></div></header>
    ...
    {selected && <>
      ...
      <section className="time-card-tools-grid">
        <form action={adminClockAction} className="time-card-card time-card-form"><h2>Administrative clock</h2><input type="hidden" name="period" value={period.start}/><input type="hidden" name="employeeId" value={selected.employee.id}/><input type="hidden" name="intent" value={selected.openPunch ? "out" : "in"}/><p className="time-card-muted">This uses the secure server time. Confirm you intend to clock {selected.employee.name} {selected.openPunch ? "out" : "in"} now.</p>{selected.openPunch && <label>Optional clock-out reason<input name="reason" maxLength={500}/></label>}<button className="time-card-small-button">Confirm clock {selected.openPunch ? "out" : "in"}</button></form>
        <form action={setPayRateAction} className="time-card-card time-card-form"><h2>Hourly rate</h2><input type="hidden" name="period" value={period.start}/><input type="hidden" name="employeeId" value={selected.employee.id}/><label>Rate in dollars<input name="hourlyRate" inputMode="decimal" placeholder="00.00" required/></label><label>Effective date<input type="date" name="effectiveDate" required/></label><label>Required reason<input name="reason" minLength={3} maxLength={500} required/></label><button className="time-card-small-button">Save effective rate</button>{selected.rates.length > 0 && <small className="time-card-muted">History: {selected.rates.map((rate) => `${currencyFromCents(rate.hourlyRateCents)} from ${rate.effectiveDate}`).join(" · ")}</small>}</form>
        ...
        <form action={addAdjustmentAction} className="time-card-card time-card-form"><h2>Manual adjustment</h2><input type="hidden" name="period" value={period.start}/><input type="hidden" name="employeeId" value={selected.employee.id}/><label>Payroll date<input type="date" name="payrollDate" min={period.start} max={period.end} required/></label><label>Hours (+ or −)<input type="number" name="hours" step="0.01" required/></label><label>Required reason<input name="reason" minLength={3} maxLength={500} required/></label><button className="time-card-small-button">Add adjustment</button></form>
      </section>
    </>}
